...
export const getUserAnalytics = async (token: string, startDate: string, endDate: string) => {
	let error = null;
	...
	const res = await fetch(
		`${WEBUI_API_BASE_URL}/litellm/user/analytics?${queryParams.toString()}`,
		{
			method: 'GET',
			headers: {
				'Content-Type': 'application/json',
...
export const getModelInfoMap = async (token: string) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/litellm/model_info`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
